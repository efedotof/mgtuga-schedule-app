import 'package:classmate/library/src/properties/day_style.dart';
import 'package:classmate/library/src/properties/easy_day_props.dart';
...
class EasyDayWidget extends StatelessWidget {
  ...
  BoxDecoration _buildDayDefaultDecoration(Color backgroundColor) {
    ...
    final todayHighlightColor =
        easyDayProps.todayHighlightColor ??
  ...
  DayInfoText _buildDayNumber(bool isToday) {
    final activeDayNumStyle =
        _activeDayStyle.dayNumStyle ??
        easyDayProps.activeDayNumStyle ??
        EasyTextStyles.dayNumStyle.copyWith(color: activeTextColor);

    final inactiveDayNumStyle =
        _inactiveDayStyle.dayNumStyle ??
        easyDayProps.inactiveDayNumStyle ??
        EasyTextStyles.dayNumStyle;

    final todayNumStyle =
        _todayStyle.dayNumStyle ??
        easyDayProps.todayNumStyle ??
        EasyTextStyles.dayNumStyle;
    final disabledNumStyle =
        _disabledDayStyle.dayNumStyle ??
        EasyTextStyles.dayNumStyle.copyWith(color: EasyColors.disabledDayColor);
    ...
    return DayInfoText(text: date.day.toString(), textStyle: dayNumStyle);
  }
  ...
  DayInfoText _buildDayString(bool isToday) {
    final activeDayStrStyle =
        _activeDayStyle.dayStrStyle ??
        easyDayProps.activeDayStrStyle ??
        EasyTextStyles.dayStrStyle.copyWith(color: activeTextColor);

    final inactiveDayStrStyle =
        _inactiveDayStyle.dayStrStyle ??
        easyDayProps.inactiveDayStrStyle ??
        EasyTextStyles.dayStrStyle;

    final todayStrStyle =
        _todayStyle.dayStrStyle ??
        easyDayProps.todayStrStyle ??
        EasyTextStyles.dayStrStyle;
    final disabledStrStyle =
        _disabledDayStyle.dayStrStyle ??
        EasyTextStyles.dayStrStyle.copyWith(color: EasyColors.disabledDayColor);
  ...
  Widget _buildDayStructure({
    ...
    switch (structure) {
      case DayStructure.dayNumberOnly:
        items = [_buildDayNumber(isToday)];
        break;
      case DayStructure.dayNameOnly:
        items = [_buildDayString(isToday)];
        break;
      case DayStructure.dayNumDayStr:
        items = [
          _buildDayNumber(isToday),
          const SizedBox(width: EasyConstants.landscapeDayPadding),
    ...
    bool isNotCompleteDate =
        (structure == DayStructure.dayNumDayStr ||
